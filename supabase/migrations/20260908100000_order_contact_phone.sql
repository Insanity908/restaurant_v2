-- Contacto adicional (opcional) que o próprio cliente pode deixar ao montar
-- o pedido pelo link público (mesa ou entrega) — distinto de customer_phone,
-- que no fluxo de entrega é sempre o telefone já registado na fidelização
-- (ver verify_loyalty_customer) e nunca existe no fluxo de mesa. Serve para
-- o restaurante poder ligar ao cliente sobre este pedido em concreto.

alter table public.orders add column if not exists contact_phone text;

drop function if exists public.submit_customer_order(uuid, uuid, text, text, jsonb, text, text);
