...
-- =============================================================================
-- 25) archived_reports — arquivamento automático de dados por ano civil
-- =============================================================================
-- Ver edge function archive-old-years. Um ano só é arquivado depois de estar
-- bem fechado (regra aplicada no código da função, não aqui): gera um .xlsx
-- do ano inteiro MAIS um .xlsx por cada mês desse ano com pedidos pagos —
-- mesmas 7 folhas que "Arquivo de Dados" já produz manualmente hoje —,
-- guarda-os neste bucket, e só DEPOIS de todos confirmados é que os dados em
-- bruto desse ano (orders — cascata automática para order_items/
-- order_events/order_payments —, shifts, security_alerts) são apagados.
-- Nunca apaga despesas/salários/clientes/catálogo/equipa/configuração —
-- mesmo âmbito que purge-old-data já respeita.
...
-- "<tenant_id>/<ano>.xlsx" ou "<tenant_id>/<ano>-<mm>.xlsx"). Só o
-- service_role escreve (a função) — sem policy de insert/update/delete para
-- `authenticated`, propositadamente: nem o admin do tenant deve poder
-- substituir/apagar um arquivo já gerado pela própria app.
insert into storage.buckets (id, name, public)
...
-- folga extra de propósito, para nunca apanhar ninguém a meio de fechar
-- contas de Janeiro/Fevereiro relativas ao ano anterior. security definer +
-- só concedida a service_role: nunca é chamável por um utilizador normal.
...
-- Nota: o agendamento pg_cron (extensões + cron.schedule + o secret via
-- Supabase Vault) NÃO está incluído aqui de propósito — vive só como
-- migração incremental (20260825201600_archive_cron_schedule.sql), porque
-- exige um passo manual de provisionamento de secret que não faz sentido
-- automatizar numa instalação de raiz. Ver essa migração para os detalhes.
